...
class RepoEnv(gym.Env):
    def __init__(self,
        ...
        # Get the logger
        if logger is None:
            self.logger = get_logger("RepoEnv", level=LOG_LEVEL_MAP[os.getenv("LOG_LEVEL", "INFO")])
        else:
            self.logger = logger
    ...
    def run_action(self, action: Action, timeout: int):
        ...
        try:
            ...
            # move this assert to rllm. need to tell model.
            # assert (
    ...
    def step(
        ...
        self.logger.debug(f"Action: {action.to_dict()} \nOutput: {bash_output} \nTotal time: {total_time}")
