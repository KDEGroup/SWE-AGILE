...
# TODO: hardcoded?  added for go and rust
DOCKER_PATH = "/root/.venv/bin:/root/.local/bin:/root/.cargo/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/go/bin:/usr/local/go/bin:/usr/local/cargo/bin:/usr/local/cargo/bin/cargo"
...
class DockerRuntime(ExecutionEnvironment):
    ...
    def __init__(
        ...
        **docker_kwargs,
        ...
        self.swebench_verified = "swebench" in self.docker_image and not "mswebench" in self.docker_image  # R2E-Gym/SWE-Bench-Verified, different from original SWE-bench/SWE-bench-verified
        self.swesmith = "swesmith" in self.docker_image  # r2e-edits/SweSmith-RL-Dataset, different from original SWE-bench/SWE-smith
        self.mswebench = "mswebench" in self.docker_image  # same as ByteDance-Seed/Multi-SWE-bench
        # otherwise R2E-Gym/R2E-Gym-Subset?
        self.r2egym = "namanjain12" in self.docker_image
        if self.swesmith:
        ...
        if logger is None:
            ...
            self.logger = get_logger(logger_name, level=logging.DEBUG)  # Pass the module name for clarity
        ...
        self.repo_name = (
            self.ds["repo"] if self.swebench_verified or self.swesmith or self.mswebench else self.ds["repo_name"]
        )
        
        # mswebench images use /home/{repo_name} instead of /testbed
        if self.mswebench:
            # Extract the repository name (e.g., "ponyc" from "ponylang/ponyc")
            repo_short_name = self.repo_name.split('/')[-1] if '/' in self.repo_name else self.repo_name
            self.repo_path = f"/home/{repo_short_name}"
        ...
        if self.backend == "docker":
            self.logger.debug(f"Container ID: {self.container.id}")
        elif self.backend == "kubernetes":
    ...
    def setup_env_mswebench(self):
        """
        Setup environment for mswebench images.
        mswebench images already contain test scripts at /home/{test-run.sh,fix-run.sh}
        Dynamically detects Python version and installs compatible dependencies.
        Some env only have python2.
        """
...
"""
        ...
        # case1: pre run test-run.sh  and save the result
        # if os.path.exists(repo_host_path):
        #     self.loguru_logger.info(f"skipping mswebench setup for {repo_host_path}")
        #     return None
        # self.mswebench_test_run_result, _ = self.run("bash /home/test-run.sh", timeout=1200)
        # if not os.path.exists(repo_host_dir):
        #     os.makedirs(repo_host_dir, exist_ok=True)
        # if "/home/test-run.sh:" in self.mswebench_test_run_result and "not found" in self.mswebench_test_run_result:
        #     self.loguru_logger.error(f"xxx not found in docker {self.docker_image}")
        #     return None
        # with open(repo_host_path, "w", encoding="utf-8") as f:
        #     f.write(self.mswebench_test_run_result)
        # return None



        # case2: load off-line test results
        if os.path.exists(repo_host_path):
    ...
    def setup_tools_uv_env(self):
        # copy and install python3.10.14
        self.run("mkdir -p /tools_uv_env/")
        self.copy_to_container(os.path.join(PROJECT_ROOT, "tools_uv_env/cpython-3.10.14+20240415-x86_64-unknown-linux-gnu-install_only.tar.gz"),
...
"""
        ...
        #     self.run("source /tools_uv_env/.venv/bin/activate")
        self.logger.debug(f"Tools UV environment ready")  
        


    def setup_env_r2egym(self):
        # setup venv
    ...
    def setup_env(self):
        ...
        if self.swebench_verified:
            ...
            return self.setup_env_mswebench()
        elif self.r2egym:
            self.setup_env_r2egym()
        else:
    ...
    def get_task_instruction(self) -> str:
        ...
        # For mswebench, add repo path information
        if self.mswebench:
            repo_info = f"\n\n[IMPORTANT] The repository code is located at: {self.repo_path}\n"
            repo_info += f"Please use this path when working with files (NOT /testbed).\n"
    ...
    def _calculate_reward_mswebench(self, get_test_output=False, timeout: int = 300) -> float:
        """
        Calculate reward for MSWebench using Report-like logic.
        
        MSWebench evaluation follows a similar pattern to multi-swe-bench:
        1. run: Execute tests without any patches (baseline)
        2. test_patch: Execute tests after applying test patch
        3. fix_patch: Execute tests after applying both test and fix patches
        
        Returns:
            1.0 if the fix is valid (fixes tests without breaking others)
            0.0 otherwise
        """
        import sys
        import os
        # Add multi-swe-bench to path
        # multi_swe_bench_path = os.path.join(PROJECT_ROOT, "multi-swe-bench-main")
        # if multi_swe_bench_path not in sys.path:
        #     sys.path.insert(0, multi_swe_bench_path)
        ...
        try:
            # self.logger.info("MSWebench: Running test-run.sh after agent execution")
            fix_run_result, run_exit_code = self.run("/home/test-run.sh", timeout=timeout)

            org = self.ds['repo'].split('/')[0]
            repo = self.ds['repo'].split('/')[1]
            instance = instance(org, repo, self.ds['number'])

            report = generate_report(instance, "", test_patch_result = self.mswebench_test_run_result, fix_patch_result = fix_run_result)
    ...
    def _detect_language_from_repo(self, repo_name: str) -> str:
        """Detect programming language from repository name or known mappings."""
        ...
        # Default to None for auto-detection
        return None
    ...
    def _calculate_reward(self, get_test_output=False, timeout: int = 300) -> float:
        if self.swebench_verified:
            ...
            return self._calculate_reward_mswebench(get_test_output=get_test_output, timeout=timeout)
        elif self.swesmith:
