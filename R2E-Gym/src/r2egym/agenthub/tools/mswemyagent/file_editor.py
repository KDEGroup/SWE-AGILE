...
class StrReplaceEditor:
    ...
    def run(
        ...
        old_content_range: List[int] = None,
        concise: bool = False,
        ...
        elif command == "range_modify":
            return self.range_modify(path, old_content_range, new_str)
        elif command == "undo_edit":
            return self.undo_edit(path)
        else:
            raise EditorError(
                f"Unrecognized command '{command}'. "
                "Allowed commands: view, create, str_replace, insert, range_modify, undo_edit."
            )
    ...
    def str_replace(self, path: Path, old_str: str, new_str: str | None) -> EditorResult:
        if not old_str:
    ...
    def range_modify(
        self, path: Path, old_content_range: List[int], new_str: str
    ) -> EditorResult:
        if not old_content_range or len(old_content_range) != 2:
            raise EditorError(
                "Missing or invalid 'old_content_range' parameter for 'range_modify'. "
                "Expected [start_line, end_line]."
            )
        if new_str is None:
            # Allow replacing with nothing (deletion)
            new_str = ""

        start_line, end_line = old_content_range

        # 1-based indexing validation
        if start_line < 1:
            raise EditorError(
                f"Invalid old_content_range: start_line must be >= 1, got {start_line}."
            )

        old_text = self.read_file(path)
        file_text_lines = old_text.split("\n")
        total_lines = len(file_text_lines)

        if start_line > total_lines:
            raise EditorError(
                f"Invalid old_content_range: start_line ({start_line}) is after the end of the file ({total_lines} lines)."
            )

        # Handle end_line == -1 (to end of file)
        if end_line == -1:
            end_line = total_lines

        if end_line < start_line:
            raise EditorError(
                f"Invalid old_content_range: end_line ({end_line}) cannot be before start_line ({start_line})."
            )

        if end_line > total_lines:
            raise EditorError(
                f"Invalid old_content_range: end_line ({end_line}) is after the end of the file ({total_lines} lines)."
            )

        # Convert to 0-based indexing for slicing
        start_idx = start_line - 1
        end_idx = end_line  # Slicing is exclusive, so this correctly identifies the line *after* end_line

        new_str_lines = new_str.splitlines()

        # Reconstruct the file
        # Lines before the change + new lines + lines after the change
        new_file_text_lines = (
            file_text_lines[:start_idx]
            + new_str_lines
            + file_text_lines[end_idx:]
        )
        ...
        # Snippet logic
        # Show a few lines before, the new content, and a few lines after
        snippet_start_line_idx = max(0, start_idx - SNIPPET_LINES)
        
        snippet_lines = (
            file_text_lines[snippet_start_line_idx:start_idx]
            + new_str_lines
            + file_text_lines[end_idx : end_idx + SNIPPET_LINES]
        )
        snippet = "\n".join(snippet_lines)
        ...
        success_msg += self._make_output(
            snippet,
            "a snippet of the edited file",
            snippet_start_line_idx + 1, # Convert 0-based index back to 1-based line num
        )
        success_msg += (
            "Review the changes and make sure they are as expected "
            "(correct indentation, no duplicate lines, etc). Edit the file again if necessary."
...
def main():
    def parse_line_range(range_str: str):
    ...
    parser = argparse.ArgumentParser(
        description=(
            "A disk-backed file editing tool (view, create, str_replace, insert, range_modify, undo_edit) "
    ...
    parser.add_argument(
        "command",
        type=str,
        help="One of: view, create, str_replace, insert, range_modify, undo_edit",
    )
    parser.add_argument(
        "--path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="New string (for 'str_replace', 'insert', or 'range_modify')",
    )
    parser.add_argument(
        "--insert_line",
        type=int,
    ...
    parser.add_argument(
        ...
        type=parse_line_range,
        default=None,
        help="Line range to replace [start_line, end_line] (for 'range_modify'), use -1 for end.",
    )
    parser.add_argument(
        "--enable_linting",
        type=bool,
    ...
    try:
        result = editor.run(
            ...
            old_content_range=args.old_content_range,
