...
def convert_messages_to_tokens_and_masks(messages: list[dict[str, str]], tokenizer: PreTrainedTokenizerBase, parser: ChatTemplateParser, contains_first_msg=False, contains_generation_msg=False, last_n_reasoning=0):
    """
    ...
    def _convert_message_to_tokens_and_masks(msg, first_msg=False, generation_msg=False):
        # last_n_reasoning matters only when digest only or full CoT. mode==Token
        msg_text = parser.parse([msg], add_generation_prompt=generation_msg, is_first_msg=first_msg, last_n_reasoning=last_n_reasoning)
