...
from rllm.engine.rollout.rollout_engine import ModelOutput, TaskStuck
...
class AgentExecutionEngine:
    def __init__(
        ...
        env_args=None,
        max_workers=10,
        enforce_max_prompt_length=False,  # If enabled, applies max_prompt check per step. In agent_ppo_trainer: enforce_max_prompt_length=self.config.rllm.stepwise_advantage.enable
        ...
        **kwargs,
        ...
        # seems only for execute_tasks, not for trajectory_generator since rllm/trainer/verl/agent_ppo_trainer.py invoked update_envs_and_agents
        ...
        self.env_timeout = self.config.rllm.agent.get("env_timeout", 375)
        ...
        sampling_params_raw = config.sampling_params
        if isinstance(sampling_params_raw, OmegaConf):
            sampling_params = OmegaConf.to_container(sampling_params_raw, resolve=True) or {}
        else:
            sampling_params = dict(sampling_params_raw) if sampling_params_raw else {}
    ...
    async def run_agent_trajectory_async(self, idx, application_id, seed=0, mode="Text", **kwargs):
        ...
        # Use 'is not None' to correctly handle last_n_reasoning=0
        if self.config.rllm.get('last_n_reasoning', None) is not None:
            last_n_reasoning = self.config.rllm.get('last_n_reasoning')
            kwargs['last_n_reasoning'] = last_n_reasoning
        else:
            import random
        ...
        # for step return
        episode_steps = []
        step_reasoning_lengths = []  # reasoning length for each step
        step_response_lengths = []  # response length for each step

        # Reset environment with the task using the executor
        loop = asyncio.get_event_loop()
        # TODO: why sometimes Container start error? maybe concurrency issue
        ...
        prompt_tokens, _ = convert_messages_to_tokens_and_masks(messages, tokenizer=self.tokenizer, parser=self.chat_parser, contains_first_msg=True, contains_generation_msg=True, last_n_reasoning=kwargs['last_n_reasoning'])
        ...
        if prompt_token_len > self.max_prompt_length:
            ...
            raise PromptTooLongError(f"Trajectory {idx}: Initial prompt length {prompt_token_len} exceeds max_prompt_length {self.max_prompt_length}. Skipping this sample entirely (no trajectory will be returned). First 200 chars of prompt: {self.chat_parser.parse(messages[:1], add_generation_prompt=False)[:200]}...")
        ...
        for step_idx in range(self.max_steps):
            ...
            if not self.enforce_max_prompt_length:
                ...
                prompt_str = self.chat_parser.parse(prompt_messages, add_generation_prompt=True, is_first_msg=True, last_n_reasoning=last_n_reasoning)
            ...
            # Update steps
            # last_n_reasoning shoule be consistent in prompt and prompt_ids
            # step-wise RL and SFT traj collecting need mode=='STEP'
            prompt_response_pair = {
                "prompt": self.chat_parser.parse(prompt_messages, add_generation_prompt=True, is_first_msg=True, last_n_reasoning=kwargs['last_n_reasoning']),
                "response": response,
            ...
            # Detect consecutive repeated actions (same tool + parameters) for last 3 steps.
            cur_step = agent.get_current_state()

            # Collect reasoning and response lengths for this step
            reasoning_text = cur_step.reasoning if cur_step.reasoning else ""
            reasoning_len = len(self.tokenizer.encode(reasoning_text, add_special_tokens=False)) if reasoning_text else 0
            response_len = len(self.tokenizer.encode(response, add_special_tokens=False))
            step_reasoning_lengths.append(reasoning_len)
            step_response_lengths.append(response_len)

            if self.config.rllm.get('check_repeating', True) and len(agent.trajectory.steps) >= 3:
                prev1_step = agent.trajectory.steps[-2]
                prev2_step = agent.trajectory.steps[-3]
                if agent.same_action(cur_step, prev1_step) and agent.same_action(cur_step, prev2_step):
                    termination_reason = "REPEATING"
                    cur_step.done = True
                    cur_step.reward = 0.0
                    done = True
                    logger.debug(f"action 1: {prev2_step.action}\naction 2: {prev1_step.action}\ncur action: {cur_step.action}\n")
                    break

            # Check format error immediately after model response
            if self.config.rllm.get('check_format', True) and (cur_step.reasoning is None or cur_step.reasoning == ""):
                termination_reason = "FORMAT_ERR"
                cur_step.done = True
                cur_step.reward = 0.0
                done = True
                break
            ...
            try:
                # for SWEAgent, returned reward here is currently always 0.
                next_observation, reward, done, info = await asyncio.wait_for(loop.run_in_executor(self.executor, env.step, action, agent), timeout=(self.trajectory_timeout - total_time))
        ...
        # Does response_masks = [0] * len(response_masks) mean no negative reward
        masked_out = False
        if self.overlong_filter:
            if termination_reason == "TRUNCATION" or termination_reason == "PROMPT_TRUNCATION" or termination_reason == "MAX_STEPS" or termination_reason == "TIMEOUT":
        ...
        if hasattr(env, "compute_final_reward") and not masked_out:
            target = OmegaConf.select(self.config, "rllm.run.target", default=None)
            if target == "eval" or termination_reason == "ENV_DONE":    # Evaluation: count more success; Training: must be complete success   
                cur_step = agent.get_current_state()
                start_time = time.time()
        ...
        agent.trajectory.last_n_reasoning = last_n_reasoning
        ...
        full_trajectory_token_length = len(self.tokenizer.encode(self.chat_parser.parse(messages, is_first_msg=True, tools=tools, last_n_reasoning=999), add_special_tokens=False)) # with all reasoning content
        actual_trajectory_token_length = len(self.tokenizer.encode(self.chat_parser.parse(messages, is_first_msg=True, tools=tools, last_n_reasoning=last_n_reasoning), add_special_tokens=False))
        compression_rate = 1 - (actual_trajectory_token_length / full_trajectory_token_length)
        ...
        if mode == "Text":
            ...
            token_result = {
                ...
                "metrics": {
                    ...
                    "full_trajectory_token_length": full_trajectory_token_length,
                    "actual_trajectory_token_length": actual_trajectory_token_length,
                    ...
                    # Per-step metrics
                    "step_reasoning_lengths": step_reasoning_lengths,
                    "step_response_lengths": step_response_lengths,
                },
            }
            return token_result
    ...
    async def run_agent_trajectory_with_retry(self, idx, seed=0, mode="Text", **kwargs):
        for i in range(self.retry_limit):
            try:
                return await asyncio.wait_for(self.run_agent_trajectory_async(idx, seed=seed, mode=mode, **kwargs), timeout=None)
            except PromptTooLongError as e:
                return None
            except Exception as e:
                logger.error(traceback.format_exc())
                if isinstance(e, TaskStuck) and i == (self.retry_limit - 1):
                    return None
                continue
        logger.error(traceback.format_exc())
        return None
    ...
    async def trajectory_generator(self, reset_seed=0, timing_raw=None, mode="Text", **kwargs):
        ...
        # Recreate executor if it was shutdown in a previous iteration
    ...
    async def execute_tasks(self, tasks: list[dict], **kwargs):
        """
        ...
        Returns:
            A list of dicts, one for each task.
        """
        if not hasattr(self, "executor") or self.executor._shutdown:
            self.executor = ThreadPoolExecutor(max_workers=self.max_env_workers)
        ...
        async def sem_wrapper(task_id, task):
            ...
            async with semaphore:
                ...
                try:
                    ...
                    self.agent_args = dict(self.config.rllm.agent.get("agent_args", {}))
                    self.agent_args["language"] = task.get("language", "python")
                    self.agents[index] = self.agent_class(chat_template_parser=ChatTemplateParser.get_parser(self.tokenizer), **self.agent_args)
                    ...
                    res = await self.run_agent_trajectory_with_retry(index, application_id=task_id, mode=kwargs.get("mode", "Text"))          
                    # debug:
                    # if not res==None and isinstance(res, Trajectory) and res.reward > 0:
                    #     import sys
                    #     print(res)
                    #     sys.exit()   
                    # if not res==None and kwargs.get("mode", "Text")=="Step" and res["trajectory_reward"] > 0:
                    #     import sys
                    #     print(res)
                    #     sys.exit() 

                    # some docker image may failed to start
                    if res is None:
                        return None
                    
                    completed += 1
                    # Count success and report running success rate every 50 trajectories.
                    try:
                        if kwargs.get("mode", None) == "Text":
                            reward_val = getattr(res, "reward", 0)
                        else:
                            reward_val = res.get("trajectory_reward", 0) if isinstance(res, dict) else getattr(res, "reward", 0)
                    except Exception:
                        reward_val = 0
                    ...
                    if completed % 50 == 0:
                        colorful_print(
                            f"Running success rate: {succeeded}/{completed} = {succeeded / completed:.2%}",
                            "cyan",
                        )
                    ...
                    #     self.logs_dir = f"logs/{t}"
                    # os.makedirs(self.logs_dir, exist_ok=True)
                    # with open(f"{self.logs_dir}/{task_id}.json", "w") as f:
                    #     json.dump(res.to_dict(), f)
                    
                    if kwargs.get("mode", None) == "Token":
                        res["task"] = task
                        res["language"] = task.get("language", "python")
                        res.pop("prompt_tokens")
                        res.pop("response_tokens")
                        res.pop("response_masks")
                    elif kwargs.get("mode", None) == "Text":
                        res.task = task
        ...
        results = {task_id: res for task_id, res in filter(None, results)}
        ordered_result = [result for task_id, result in sorted(results.items())]
