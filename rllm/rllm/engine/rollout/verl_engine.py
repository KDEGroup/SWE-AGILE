...
class VerlEngine(RolloutEngine):
    ...
    async def get_model_response(self, messages: list[dict], **kwargs) -> ModelOutput:
        ...
        last_n_reasoning = kwargs.pop('last_n_reasoning')
        ...
        # Note: vLLM's SamplingParams directly supports repetition_penalty, no need to wrap in extra_body
        # (extra_body is only for OpenAI client API, not for direct vLLM SamplingParams)
        if sampling_params.get('repetition_penalty') and sampling_params["repetition_penalty"] > 1:
            logger.debug(f"repetition_penalty: {sampling_params['repetition_penalty']}")
        prompt = self.chat_parser.parse(messages, add_generation_prompt=True, is_first_msg=True, tools=tools, accumulate_reasoning=accumulate_reasoning, last_n_reasoning=last_n_reasoning, **kwargs)
        # logger.debug(f"prompt in VerlEngine.get_model_response(): {prompt}")
        ...
        if not parsed_output["reasoning"]:
            if self.config.rllm.get('check_format', True):
                print(f"no reasoning parsed! llm finish_reason: {finish_reason}\n")
            # print(f"text: {completion_text}")
