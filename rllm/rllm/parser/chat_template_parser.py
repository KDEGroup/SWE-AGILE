...
class QwenChatTemplateParser(ChatTemplateParser):
    def __init__(self, tokenizer, processor=None, disable_thinking=False):
        ...
        # self.think_token_begin = "<reasoning>"
        # self.think_token_end = "</reasoning>"
        self.think_token_begin = "<think>"
    ...
    def parse(self, messages: list[dict], add_generation_prompt: bool = False, is_first_msg: bool = False, tools: list[Tool] = None, accumulate_reasoning: bool = False, **kwargs) -> str:
        """
        reasoning content is not included in messages[i]["content"] but messages[i]["reasoning"]
        """
        last_n_reasoning = kwargs.get("last_n_reasoning")
        tools = tools or []
        tools_prompt_str = ""
        if tools:
            try:
        ...
        for i, message in enumerate(messages):
            if message["role"] == "system":
                ...
                if i >= len(messages) - last_n_reasoning * 2:
                    result += self.parse_assistant(message, accumulate_reasoning=True)
                else:
                    result += self.parse_assistant(message, accumulate_reasoning=False)
            elif message["role"] == "tool":
                result += self.parse_tool(message)
            else:
                raise NotImplementedError(f"Unsupported message role: {message['role']}")
    ...
    def parse_assistant(self, message, accumulate_reasoning=False):
        ...
        if not reasoning and not tool_calls:
            ...
            if reasoning and accumulate_reasoning:
                result += f"{self.think_token_begin}\n" + reasoning + f"\n{self.think_token_end}\n\n"
    ...
    def parse_completion(self, completion_ids, json_tool_call=False):
        completion_text = self.tokenizer.decode(completion_ids, skip_special_tokens=False)
        if completion_text.count(self.think_token_end) == 1:
            reasoning, _, content = completion_text.partition(self.think_token_end)
            if reasoning.startswith(self.think_token_begin):
                reasoning = reasoning[len(self.think_token_begin) :]
            if content.endswith(self.eos_token):
                content = content[: -len(self.eos_token)]
            ...
            content = content.strip()
        # handle </xxx>...</xxx>
        elif completion_text.count(self.think_token_end) == 2:
            start = completion_text.find(self.think_token_end)
            end = completion_text.rfind(self.think_token_end)
            reasoning = completion_text[(start + len(self.think_token_end)):end]
            content = completion_text[(end + len(self.think_token_end)):]
            if content.endswith(self.eos_token):
                content = content[: -len(self.eos_token)]
            if content.endswith(self.eot_token):
                content = content[: -len(self.eot_token)]
            ...
            reasoning = None
            content = completion_text
            if content.startswith(self.think_token_begin):
                content = content[len(self.think_token_begin) :]
            if content.endswith(self.eos_token):
                content = content[: -len(self.eos_token)]
            if content.endswith(self.eot_token):
                content = content[: -len(self.eot_token)]
