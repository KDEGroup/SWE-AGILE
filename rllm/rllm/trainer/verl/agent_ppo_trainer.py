...
class AgentPPOTrainer(RayPPOTrainer):
    ...
    def init_envs_and_agents(self, batch):
        ...
        loguru_logger.info(f"full_agent_args: {full_agent_args}")
        loguru_logger.info(f"Base environment arguments: {base_env_args}")
        def _create_env(i):
        ...
        def _create_agent(i):
            # add language to full_agent_args
            full_agent_args["language"] = env_args[i].get("language", "python")
            return i, self.agent_class(chat_template_parser=ChatTemplateParser.get_parser(self.tokenizer), **full_agent_args)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.rllm.stepwise_advantage.enable:
                        ...
                        # Extract trajectory metrics for logging
                        traj_metrics = final_gen_batch_output.meta_info.pop("traj_metrics", {})
                        metrics.update({f"traj/{k}": v for k, v in traj_metrics.items()})
                    ...
                    with marked_timer("adv", timing_raw):
                        ...
                        for uid in unique_uids:
                            ...
                            has_reward_difference = not torch.isclose(uid_rewards, uid_rewards[0]).all()

                            # Keep solve_none / solve_all statistics independent from masking.
                            if (uid_rewards <= 0).all():
                                solve_none += 1
                            elif (uid_rewards >= 1).all():
                                solve_all += 1

                            # Masking only depends on whether there is any reward difference
                            # within the group, independent of solve_none / solve_all stats.
                            valid_mask[uid_mask] = has_reward_difference
    ...
    def _transform_agent_trajectories(self, trajectories: list[dict]):
        ...
        for k, v_list in traj_metrics.items():
            # Special handling for per-step length lists (list of lists)
            if k in ["step_reasoning_lengths", "step_response_lengths"]:
                ...
                if flat_list:
                    ...
                    metrics.update(
                        {
                            f"traj/per_{k}_mean": flat_arr.mean(),
                            f"traj/per_{k}_min": flat_arr.min(),
                            f"traj/per_{k}_max": flat_arr.max(),
                        }
                    )
                continue
        ...
        # TODO: why not torch.bfloat16 or torch.float16?
    ...
    def _transform_agent_steps(self, steps: list[dict], uids: np.ndarray):
        ...
        all_step_training_rewards = []  # flattened step-level rewards aligned with all steps
        training_rewards = []
        all_mc_returns = []  # Monte Carlo returns for each episode
        all_compression_rates = []  # compression rate for each trajectory
        all_termination_reasons = []  # termination reasons for each trajectory
        # Metrics for non-FORMAT_ERR trajectories
        noformaterr_step_numbers = []
        noformaterr_rewards = []
        noformaterr_compression_rates = []
        noformaterr_response_lengths = []
        noformaterr_per_step_reasoning_lengths = []  # individual step reasoning lengths for non-FORMAT_ERR
        noformaterr_per_step_response_lengths = []  # individual step response lengths for non-FORMAT_ERR
        # the last step will have reward assigned and be used for advantage calculation

        # Get compression reward weight from config (0.0 means no compression reward)
        compression_reward_weight = self.config.rllm.get("compression_reward_weight", 1.0)
        max_compression_rate = self.config.rllm.get("max_compression_rate", 0.625)
        invalid_step_penalty = self.config.rllm.get("formaterr_penalty", -0.5)
        penalized_termination_reasons = {"FORMAT_ERR", "REPEATING", "ENV_TIMEOUT"}

        for episode in steps:
            episode_steps = episode["steps"]
            idx = episode["idx"]
            trajectory_reward = episode["trajectory_reward"]
            mc_returns = episode["mc_returns"]
            compression_rate = episode.get("compression_rate", 0.0)
            termination_reason = episode.get("termination_reason", None)
            ...
            # Combine trajectory reward with compression rate
            # Only add compression reward when trajectory is successful (reward > 0)
            # Clamp compression rate only for reward calculation, not for statistics
            if compression_reward_weight > 0 and trajectory_reward > 0:
                clamped_compression_rate = min(compression_rate, max_compression_rate)
                base_training_reward = trajectory_reward + compression_reward_weight * clamped_compression_rate
            else:
                base_training_reward = trajectory_reward

            # Default to the same reward for each step in the trajectory.
            step_training_rewards = [base_training_reward for _ in range(len(episode_steps))]

            # For FORMAT_ERR / REPEATING trajectories, apply extra penalty only on the
            # offending step (logical last step where termination happens), instead of
            # penalizing every step in the trajectory.
            if termination_reason in penalized_termination_reasons and step_training_rewards:
                step_training_rewards[-1] += invalid_step_penalty

            training_reward = step_training_rewards[-1] if step_training_rewards else base_training_reward

            # Calculate response length for this trajectory (sum of all step responses)
            traj_response_tokens = [self.tokenizer.encode(s["response"], add_special_tokens=False) for s in episode_steps]
            traj_response_length = sum(len(tokens) for tokens in traj_response_tokens)
            ...
            all_responses_list.extend([torch.tensor(tokens, dtype=torch.long) for tokens in traj_response_tokens])

            step_numbers.append(len(episode_steps) - 1)
            training_rewards.append(training_reward)
            all_step_training_rewards.extend(step_training_rewards)
            all_mc_returns.extend(mc_returns)
            all_compression_rates.append(compression_rate)  # Record real compression rate for statistics

            # Collect metrics for non-FORMAT_ERR trajectories
            if termination_reason != "FORMAT_ERR":
                noformaterr_step_numbers.append(len(episode_steps))
                noformaterr_rewards.append(training_reward)
                noformaterr_compression_rates.append(compression_rate)
                noformaterr_response_lengths.append(traj_response_length)
                # Collect per-step reasoning and response lengths
                step_reasoning_lens = episode.get("step_reasoning_lengths", [])
                step_response_lens = episode.get("step_response_lengths", [])
                noformaterr_per_step_reasoning_lengths.extend(step_reasoning_lens)
                noformaterr_per_step_response_lengths.extend(step_response_lens)
            ...
            # extend mask to the episode of this step
        ...
        # Place per-step rewards to the last response token of each step
        score_batch = torch.zeros_like(response_batch, dtype=torch.float32)
        mc_return_batch = torch.zeros_like(response_batch, dtype=torch.float32)
        assert len(all_step_training_rewards) == score_batch.shape[0], (
            f"Flattened step rewards should align with batch size, but got "
            f"{len(all_step_training_rewards)} and {score_batch.shape[0]}"
        )

        for step_index, step_score in enumerate(all_step_training_rewards):
            resp_len = response_lengths[step_index]
            if resp_len > 0 and resp_len <= score_batch.shape[1]:
                score_batch[step_index, resp_len - 1] = step_score
        ...
        meta_info = {
            ...
            # Trajectory-level metrics for logging
            "traj_metrics": {
                "success_rate": np.mean([1.0 if r > 0 else 0.0 for r in training_rewards]),
                "total_reward/mean": np.mean(training_rewards),
                "total_reward/max": np.max(training_rewards),
                "total_reward/min": np.min(training_rewards),
                "compression_rate/mean": np.mean(all_compression_rates),
                "compression_rate/max": np.max(all_compression_rates),
                "compression_rate/min": np.min(all_compression_rates),
                # Step count statistics (step_numbers is 0-indexed, so +1 for actual count)
                "steps/mean": np.mean([x + 1 for x in step_numbers]),
                "steps/max": np.max([x + 1 for x in step_numbers]),
                "steps/min": np.min([x + 1 for x in step_numbers]),
                # Termination reason statistics
                "termination/FORMAT_ERR": sum(1 for r in all_termination_reasons if r == "FORMAT_ERR"),
                "termination/ENV_DONE": sum(1 for r in all_termination_reasons if r == "ENV_DONE"),
                "termination/MAX_STEPS": sum(1 for r in all_termination_reasons if r == "MAX_STEPS"),
                "termination/TRUNCATION": sum(1 for r in all_termination_reasons if r == "TRUNCATION"),
                "termination/PROMPT_TRUNCATION": sum(1 for r in all_termination_reasons if r == "PROMPT_TRUNCATION"),
                "termination/TIMEOUT": sum(1 for r in all_termination_reasons if r == "TIMEOUT"),
                "termination/ENV_TIMEOUT": sum(1 for r in all_termination_reasons if r == "ENV_TIMEOUT"),
                "termination/REPEATING": sum(1 for r in all_termination_reasons if r == "REPEATING"),
                # Non-FORMAT_ERR trajectory statistics
                "NoFormaterr/count": len(noformaterr_step_numbers),
                "NoFormaterr/steps/mean": np.mean(noformaterr_step_numbers) if noformaterr_step_numbers else 0.0,
                "NoFormaterr/steps/max": np.max(noformaterr_step_numbers) if noformaterr_step_numbers else 0.0,
                "NoFormaterr/steps/min": np.min(noformaterr_step_numbers) if noformaterr_step_numbers else 0.0,
                "NoFormaterr/reward/mean": np.mean(noformaterr_rewards) if noformaterr_rewards else 0.0,
                "NoFormaterr/reward/max": np.max(noformaterr_rewards) if noformaterr_rewards else 0.0,
                "NoFormaterr/reward/min": np.min(noformaterr_rewards) if noformaterr_rewards else 0.0,
                "NoFormaterr/compression_rate/mean": np.mean(noformaterr_compression_rates) if noformaterr_compression_rates else 0.0,
                "NoFormaterr/compression_rate/max": np.max(noformaterr_compression_rates) if noformaterr_compression_rates else 0.0,
                "NoFormaterr/compression_rate/min": np.min(noformaterr_compression_rates) if noformaterr_compression_rates else 0.0,
                "NoFormaterr/response_length/mean": np.mean(noformaterr_response_lengths) if noformaterr_response_lengths else 0.0,
                "NoFormaterr/response_length/max": np.max(noformaterr_response_lengths) if noformaterr_response_lengths else 0.0,
                "NoFormaterr/response_length/min": np.min(noformaterr_response_lengths) if noformaterr_response_lengths else 0.0,
                # Per-step reasoning and response lengths for non-FORMAT_ERR trajectories
                "NoFormaterr/per_step_reasoning_length/mean": np.mean(noformaterr_per_step_reasoning_lengths) if noformaterr_per_step_reasoning_lengths else 0.0,
                "NoFormaterr/per_step_reasoning_length/max": np.max(noformaterr_per_step_reasoning_lengths) if noformaterr_per_step_reasoning_lengths else 0.0,
                "NoFormaterr/per_step_reasoning_length/min": np.min(noformaterr_per_step_reasoning_lengths) if noformaterr_per_step_reasoning_lengths else 0.0,
                "NoFormaterr/per_step_response_length/mean": np.mean(noformaterr_per_step_response_lengths) if noformaterr_per_step_response_lengths else 0.0,
                "NoFormaterr/per_step_response_length/max": np.max(noformaterr_per_step_response_lengths) if noformaterr_per_step_response_lengths else 0.0,
                "NoFormaterr/per_step_response_length/min": np.min(noformaterr_per_step_response_lengths) if noformaterr_per_step_response_lengths else 0.0,
            }
        }
